...
class CreateFolioRequest(BaseModel):
    ...
    CreateFolioRequest
    """ # noqa: E501
    name: StrictStr = Field(description="Folio name")
    folio_configuration_id: Optional[StrictInt] = Field(default=None, description="Folio configuration ID", alias="folioConfigurationId")
    parent_id: Optional[StrictInt] = Field(default=None, description="Parent folio ID (for sub-folios)", alias="parentId")
    source_id: StrictInt = Field(description="Source ID (reservation ID, group profile ID, or house account ID)", alias="sourceId")
    source_kind: FolioSourceKind = Field(alias="sourceKind")
    transaction_types: List[TransactionTypeIdentifier] = Field(description="Transaction types to assign to this folio", alias="transactionTypes")
    __properties: ClassVar[List[str]] = ["name", "folioConfigurationId", "parentId", "sourceId", "sourceKind", "transactionTypes"]
