...
ANDORGROUPANDINNER_ONE_OF_SCHEMAS = ["AndOrGroup", "FieldFilter"]

class AndOrGroupAndInner(BaseModel):
    """
    AndOrGroupAndInner
    """
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = AndOrGroupAndInner.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in AndOrGroupAndInner with oneOf schemas: AndOrGroup, FieldFilter. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in AndOrGroupAndInner with oneOf schemas: AndOrGroup, FieldFilter. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into AndOrGroupAndInner with oneOf schemas: AndOrGroup, FieldFilter. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into AndOrGroupAndInner with oneOf schemas: AndOrGroup, FieldFilter. Details: " + ", ".join(error_messages))
        else:
            return instance
...
AndOrGroupAndInner.model_rebuild(raise_errors=False)
