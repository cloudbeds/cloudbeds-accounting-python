...
class ListFolioTransactionsPaginated(BaseModel):
    """
    ListFolioTransactionsPaginated
    """ # noqa: E501
    groups: Optional[Dict[str, FolioTransactionGroup]] = Field(default=None, description="Ordered map of groupKey to group. Key is the string value of the groupBy field (e.g. \"2026-02-25\" for transaction_date). Pagination is applied first, then grouping. ")
    ...
    totals: Optional[FolioTransactionTotals] = None
    foreign_currency_totals: Optional[ForeignCurrencyTotals] = Field(default=None, alias="foreignCurrencyTotals")
    __properties: ClassVar[List[str]] = ["groups", "nextPageToken", "totals", "foreignCurrencyTotals"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of foreign_currency_totals
        if self.foreign_currency_totals:
            _dict['foreignCurrencyTotals'] = self.foreign_currency_totals.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "groups": dict(
                (_k, FolioTransactionGroup.from_dict(_v))
                for _k, _v in obj["groups"].items()
            )
            if obj.get("groups") is not None
            else None,
            "nextPageToken": obj.get("nextPageToken"),
            "totals": FolioTransactionTotals.from_dict(obj["totals"]) if obj.get("totals") is not None else None,
            "foreignCurrencyTotals": ForeignCurrencyTotals.from_dict(obj["foreignCurrencyTotals"]) if obj.get("foreignCurrencyTotals") is not None else None
        })
        return _obj
